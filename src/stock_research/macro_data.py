...
def update_macro_data(folder: Path, start_default: str = "1970-01-01") -> list[Path]:
    ...
    outputs: list[Path] = []
    series_to_run = {**MACRO_SERIES, "YieldCurve": YIELD_CURVE_SERIES}
    for name, code in series_to_run.items():
        ...
        combined = (
            combined.dropna(subset=["Date"]).drop_duplicates("Date", keep="last")
...
def load_vix(folder: Path) -> pd.DataFrame:
    ...
    frame = read_csv_fallback(hits[0])
    date_col = frame.columns[0]
    value_col = "Value" if "Value" in frame.columns else frame.columns[1]
    frame = frame[[date_col, value_col]].rename(
        columns={date_col: "날짜", value_col: "VIX"}
    )
    frame["날짜"] = pd.to_datetime(frame["날짜"], errors="coerce")
    frame["VIX"] = pd.to_numeric(frame["VIX"], errors="coerce")
    frame = frame.dropna(subset=["날짜"]).sort_values("날짜")
    duplicates = frame["날짜"].duplicated(keep=False)
    if duplicates.any():
        dates = frame.loc[duplicates, "날짜"].dt.strftime("%Y-%m-%d").unique()
        raise ValueError(f"Duplicate actual VIX dates: {', '.join(dates[:5])}")
    return frame
